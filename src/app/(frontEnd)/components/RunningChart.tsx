...
const RunningChart = (props: Props) => {
  ...
  const { xAxis, yAxis } = calculateXandYAxis(sortedRunningData, startDate);

  const runningData = xAxis.map((date: any) => {
    return {
    ...
    <div className="w-full h-full relative">
      ...
      <LocalizationProvider dateAdapter={AdapterDayjs}>
        <div className="mb-4 flex justify-between items-center px-0">
          <h2 className="text-sm font-semibold">Running Tracking</h2>
          ...
          >
            ...
            <YAxis
              ...
              ticks={Array.from(
                {
                  length: Math.ceil(
                    Math.max(...runningData.map((d: any) => d.uv || 0)) - 73 + 1
                  ),
                },
                (_, i) => 73 + i
              )}
...
const calculateXandYAxis = (sortedRunningData: any, startDate: any) => {
  const formattedDates = sortedRunningData.map((item: any) =>
    formatDateFromIsoString(item.properties.Date.date.start)
  );

  const xAxis = formattedDates;
  const yAxis = sortedRunningData.map((item: any) => {
    return item?.properties?.distance?.number || null;
  });
...
const getDatesFromRunning = (running: any): { runningDates: Date[] } => {
  const runningDates: Date[] = [];

  running.forEach((running: any) => {
    if (running?.properties?.["Date"]?.date?.start) {
      runningDates.push(new Date(running.properties.Date.date.start));
    }
  });
